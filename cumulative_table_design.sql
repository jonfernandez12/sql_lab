create schema cumulative_table_design;

CREATE TABLE cumulative_table_design.events (
    user_id INTEGER,
    event_type VARCHAR,
...
INSERT INTO cumulative_table_design.events (user_id, event_type, event_timestamp, event_date)
VALUES 
...
select * from cumulative_table_design.events order by event_date;


CREATE TABLE cumulative_table_design.active_users_daily (
    user_id INTEGER,
...
INSERT INTO cumulative_table_design.active_users_daily
SELECT
    user_id,
...
FROM cumulative_table_design.events
WHERE event_date = '2023-12-13'
GROUP BY user_id;

delete from cumulative_table_design.active_users_daily;
select * from cumulative_table_design.active_users_daily;

--separet table open below table query 

CREATE TABLE cumulative_table_design.active_users_cumulated (
    user_id INTEGER,
...
select * from cumulative_table_design.active_users_cumulated order by snapshot_date;




INSERT INTO cumulative_table_design.active_users_cumulated
-- First read in yesterday's data from the cumulated table
WITH yesterday AS (
    SELECT * 
    FROM cumulative_table_design.active_users_cumulated
    WHERE snapshot_date = '2023-12-12'
),
-- Read in today's active user data from the daily table
today AS (
    SELECT * 
    FROM cumulative_table_design.active_users_daily
...
combined AS (
    SELECT
       ...
       COALESCE(t.snapshot_date,'2023-12-13') as snapshot_date   
